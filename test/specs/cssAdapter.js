...
savor
  ...
  .add(
    ...
    async (context, done) => {
      const css = new CssAdapter()
      savor.addAsset('assets/test.css.remote', 'test.css.remote', context)

      const assetFileStream = fs.createReadStream(
        path.resolve(context.dir, 'test.css.remote')
      )

      const returnedFileStream = await css.process(assetFileStream)
